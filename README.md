...
# 🗓️ Project 기능
  - **서울시 공공 Wi-Fi 위치정보 API 요청**
    - "Open Api 와이파이 정보 가져오기" 클릭해 서울시 공공 WIFI 위치정보 API 를 요청

  - **서울시 공공 WIFI 위치정보 저장**
    - Json 으로 받아온 서울시 공공 WIFI 위치정보를 로컬 DB 에 저장
  - **사용자 위치기반 주변 공공 Wi-Fi 정보 검색**
    - 내 위치의 위도, 경도를 가져와서 내 주변 공공 와이파이 정보 20개를 DB에서 가져와 메인 페이지에 출력
  - **주변 공공 Wi-Fi 정보 검색 이력 조회 / 삭제**
    - 사용자 주변 공공 Wi-Fi 검색했던 위치 이력을 조회 및 삭제
  - **내 주변 공공 Wi-Fi 즐겨찾기(북마크) 기능**
    - 북마크(즐겨찾기) 생성 / 삭제 기능 및 북마크(즐겨찾기) 그룹 생성 / 수정 / 삭제 기능
